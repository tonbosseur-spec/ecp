...
export default function ClientTrainingHub() {
  ...
  // Modal notice for R exercises (coming soon)
  const [rModalNoticeOpen, setRModalNoticeOpen] = useState(false);
  const [selectedRSession, setSelectedRSession] = useState<TrainingSession | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pb-12">
                    ...
                    <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between gap-3">
                      ...
                      {isRExercise ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-2xl font-bold text-xs bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200 transition-all"
                        >
                        ...
                        <button
                          onClick={() => handleStartSession(session)}
                          className={`inline-flex items-center gap-1.5 px-4 py-2.5 rounded-2xl font-bold text-xs transition-all active:scale-95 ${
                            stats?.isPassed
                        ...
                        >
                          ...
                          <span>{stats ? "S'entraîner à nouveau" : "Commencer"}</span>
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {/* R Exercise Coming Soon Modal Notice */}
      {rModalNoticeOpen && (
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            <div className="text-center space-y-2">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-purple-50 text-purple-700 border border-purple-200">
                Environnement Interactif R
              </span>
              <h3 className="text-lg font-bold text-gray-900">
                {selectedRSession?.title || "Exercices de programmation R"}
              </h3>
              <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">
                Le moteur d'exécution interactif de code R (bac à sable sécurisé et tests automatisés) est actuellement en cours de déploiement. 
                <br /><br />
                En attendant, vous pouvez vous entraîner sur l'ensemble des quiz interactifs QCM !
              </p>
            </div>
            ...
            <div className="pt-2">
              <button
                onClick={() => {
                  setRModalNoticeOpen(false);
                  setSelectedRSession(null);
                }}
                className="w-full py-3 bg-gray-900 hover:bg-black text-white rounded-2xl font-bold text-xs sm:text-sm transition-all"
              >
                Compris, merci !
              </button>
            </div>
