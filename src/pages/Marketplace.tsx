...
export default function Marketplace() {
  ...
  const [catalogueTab, setCatalogueTab] = useState<'all' | 'classic' | 'interactive'>('all');
  ...
  const filteredInteractiveCourses = interactiveCourses.filter(course => {
    const query = searchQuery.toLowerCase();
    const titleMatch = course.title?.toLowerCase().includes(query);
    const descriptionMatch = course.description?.toLowerCase().includes(query);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        try {
          const { data: interactiveData, error: interactiveErr } = await supabase
            .from('interactive_courses')
            .select('*, interactive_course_modules(id)')
            .order('created_at', { ascending: false });

          if (interactiveErr) {
            console.error("Erreur chargement cours interactifs:", interactiveErr);
          } else if (interactiveData) {
            // Filter published courses (or non-draft courses)
            ...
            setInteractiveCourses(activeInteractive.length > 0 ? activeInteractive : interactiveData);
          }
        } catch (iErr) {
          console.error("Erreur chargement cours interactifs:", iErr);
        }
      } catch (err) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="mb-10 max-w-2xl mx-auto space-y-4">
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder="Rechercher une formation ou cours interactif..."
              value={searchQuery}
          ...
          <div className="w-full flex justify-center">
            <div className="w-full sm:w-auto flex bg-gray-100/90 p-1 rounded-xl sm:rounded-2xl gap-1 text-[10px] sm:text-xs font-bold overflow-x-auto no-scrollbar border border-gray-200/70 shadow-2xs max-w-full">
              <button
                onClick={() => setCatalogueTab('all')}
                className={`shrink-0 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                  catalogueTab === 'all'
              ...
              <button
                onClick={() => setCatalogueTab('classic')}
                className={`shrink-0 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                  catalogueTab === 'classic'
                    ? 'bg-white text-purple-700 shadow-sm'
                    : 'text-gray-600 hover:text-gray-900'
                }`}
              >
                <Users className="w-3.5 h-3.5 text-purple-600 shrink-0" />
                <span className="hidden sm:inline">Formations & Ateliers</span>
                <span className="sm:hidden">Formations</span>
                <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-extrabold ${
                  catalogueTab === 'classic' ? 'bg-purple-100 text-purple-800' : 'bg-gray-200 text-gray-700'
                }`}>
                  {courses.length}
                </span>
              </button>
              ...
              <button
                onClick={() => setCatalogueTab('interactive')}
                className={`shrink-0 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                  catalogueTab === 'interactive'
                    ? 'bg-white text-sky-700 shadow-sm'
                    : 'text-gray-600 hover:text-gray-900'
                }`}
              >
                <Sparkles className="w-3.5 h-3.5 text-sky-500 shrink-0" />
                <span className="hidden sm:inline">Cours Interactifs R</span>
                <span className="sm:hidden">Interactifs</span>
                <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-extrabold ${
                  catalogueTab === 'interactive' ? 'bg-sky-100 text-sky-800' : 'bg-gray-200 text-gray-700'
                }`}>
        ...
        {/* Catalogue Content Rendering */}
        {((catalogueTab === 'classic' && courses.length === 0) ||
          (catalogueTab === 'interactive' && interactiveCourses.length === 0) ||
          (catalogueTab === 'all' && courses.length === 0 && interactiveCourses.length === 0)) ? (
          <div className="bg-white rounded-3xl p-10 text-center border border-gray-100 shadow-sm max-w-lg mx-auto">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              Il n'y a actuellement aucun contenu disponible dans cette rubrique.
            </p>
          </div>
        ) : ((catalogueTab === 'classic' && filteredCourses.length === 0) ||
             (catalogueTab === 'interactive' && filteredInteractiveCourses.length === 0) ||
             (catalogueTab === 'all' && filteredCourses.length === 0 && filteredInteractiveCourses.length === 0)) ? (
          ...
          >
            {/* 1. COURS INTERACTIFS */}
            {(catalogueTab === 'all' || catalogueTab === 'interactive') && 
              filteredInteractiveCourses.map((course) => {
                ...
                return (
                  <motion.div
                    key={`interactive-${course.id}`}
                    variants={{
                      hidden: { opacity: 0, y: 20 },
                      visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 100, damping: 15 } }
                    }}
                    className="bg-white rounded-3xl overflow-hidden border border-sky-100 shadow-xs hover:shadow-xl hover:border-sky-300 transition-all duration-300 flex flex-col justify-between group"
                  >
                    <div>
                      <div className="relative h-48 w-full overflow-hidden bg-gradient-to-br from-sky-600 via-indigo-600 to-purple-700">
                        ...
                        <div className="absolute top-4 left-4 flex flex-col items-start gap-1">
                          <span className="px-3 py-1 rounded-full text-[11px] font-black uppercase bg-sky-600 text-white shadow-xs flex items-center gap-1">
                            <Sparkles className="w-3 h-3" /> Cours Interactif
                          </span>
                        </div>

                        <div className="absolute bottom-3 right-3 px-3 py-1 rounded-full text-xs font-black bg-black/60 backdrop-blur-md text-white border border-white/20">
                          {formattedPrice}
                        </div>
                      </div>
                      ...
                      <div className="p-6 space-y-3">
                        <div className="flex items-center justify-between text-xs font-semibold text-gray-500">
                          <span className="px-2.5 py-0.5 rounded-lg bg-sky-50 text-sky-700 border border-sky-100 font-bold">
                            {course.category || 'Logiciel R'}
                          </span>
                          <span>{course.level === 'beginner' ? 'Débutant' : course.level === 'intermediate' ? 'Intermédiaire' : 'Avancé'}</span>
                        </div>

                        <h3 className="text-lg font-extrabold text-gray-900 group-hover:text-sky-600 transition-colors line-clamp-2 leading-snug">
                          {course.title}
                        </h3>
                        ...
                        <div className="pt-2 flex items-center gap-2 text-xs font-medium text-emerald-800 bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-100">
                          ...
                          <span>{modulesCount} chapitres autonomes WebR</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-6 pt-0 mt-auto">
                      <Link
                        to={`/interactive-course/${course.slug || course.id}`}
                        className="w-full py-3 px-4 bg-sky-600 hover:bg-sky-700 text-white font-black rounded-xl text-xs transition-all flex items-center justify-center gap-2 shadow-xs group-hover:scale-[1.01]"
                      >
                        <span>{isFree ? 'Commencer gratuitement' : 'Découvrir le cours'}</span>
            ...
            {/* 2. FORMATIONS CLASSIQUES */}
            {(catalogueTab === 'all' || catalogueTab === 'classic') &&
