...
async function test() {
  ...
  console.log('Testing update for:', sessionId);
  const { data, error } = await supabase.from('live_sessions').update({ title: 'Session Live test update' }).eq('id', sessionId).select();
