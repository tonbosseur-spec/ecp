-- Migration to preserve training exercise history upon editor updates
-- Add is_active flag to training_exercises if not present
ALTER TABLE public.training_exercises 
ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true;

CREATE INDEX IF NOT EXISTS idx_training_exercises_active 
ON public.training_exercises(training_session_id, is_active);

-- Update submit_training_qcm_attempt RPC to filter only active QCM exercises for new attempts
...
BEGIN
    ...
    -- 5. Évaluation de chaque question active avec la table interne sécurisée
    ...
    LOOP
        ...
        -- Construction du feedback post-soumission
